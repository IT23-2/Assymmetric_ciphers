...
# Импортируем необходимые модули.Add commentMore actions
    ...
    try:
        ...
        while server_running:  # Цикл продолжается, пока сервер работает.
            try:
                ...
                send_message_to_client(conn, client_public_key,
                                       response)  # Используем вспомогательную функцию для отправки.
        ...
        if command.lower() == 'stop':
            ...
            with clients_lock:  # Используем мьютекс для безопасного доступа к списку клиентов.
                ...
                for client_conn, client_addr, client_key in clients_to_send:
                    ...
                    if not success:
                        print(
                            f"Отправка сообщения клиенту {client_addr} не удалась. Обработчик клиента должен отключиться сам.")
