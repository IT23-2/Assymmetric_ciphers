# input_file_0.py (Client)Add commentMore actions
...
from Crypto.Random import \
    get_random_bytes  # Модуль для генерации криптографически стойких случайных байтов (например, для сессионных ключей и nonce).
    ...
    try:
        while client_running:  # Цикл будет продолжаться, пока клиент работает.
            try:
                ...
                key_size = int.from_bytes(key_size_bytes,
                                          'big')  # Преобразуем байты размера в целое число (big-endian порядок байтов).
                ...
                tag = encrypted_msg[
                      -16:]  # Тег (Authentication Tag) - последние 16 байт, используется для проверки целостности
                ...
                cipher_aes = AES.new(session_key, AES.MODE_EAX,
                                     nonce=nonce)  # Создаем объект AES для расшифровки с сессионным ключом и Nonce.
    ...
    try:
        ...
        while client_running:  # Продолжаем, пока клиент активен (флаг client_running True).
            ...
            cipher_rsa = PKCS1_OAEP.new(
                server_public_key)  # Создаем объект для RSA-шифрования, используя публичный ключ сервера.
            ...
            ciphertext, tag = cipher_aes.encrypt_and_digest(
                message.encode())  # Шифруем сообщение (преобразуем в байты) и получаем тег.
            ...
            try:
                ...
                sock.sendall(data)
                    ...
                    OSError) as e:  # Ловим ошибки, если соединение было разорвано во время отправки.
